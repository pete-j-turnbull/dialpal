...
import { Video as RawVideo } from "@packages/video-templates";
import {
  Template,
  Video,
  videoSchema,
} from "@packages/video-templates/thought-leadership-916";
...
const generateVideoQueue = queue({
  name: "flows/thought_leadership_916/generate-video",
  concurrencyLimit: 10,
});

const generateBRoll = schemaTask({
  id: "flows/thought_leadership_916/generate-broll",
  schema: z.object({
    video: videoSchema,
    productDatasetId: z
      .string()
      .optional()
      .transform((v) => v as Id<"product_datasets"> | undefined),
  }),
  maxDuration: 300,
  retry: {
    maxAttempts: 1,
  ...
  machine: "medium-1x",
  run: async (payload) => {
    const { video, productDatasetId } = payload;

    const updatedVideo = await autoSelectBroll({
      video,
      productDatasetId,
    });
...
// TODO: generate title too
export const generateTemplate = schemaTask({
  id: "flows/thought_leadership_916/generate-template",
  schema: z.object({
  ...
  run: async (payload) => {
    ...
    const requiredData = await convex.query(
      api.private.flows.thought_leadership_916.fetchRequiredDataForTemplate,
      { id: flowId }
    );
    ...
    const rawScenes = await generateRawScenes(tokens, "Company Name");
    ...
    const template = Template.fromRaw(rawVideo);

    await convex.mutation(api.private.flows.thought_leadership_916.update, {
      id: flowId,
      data: { templatedVideo: template.video },
    });

    const updatedVideo = await generateBRoll
      .triggerAndWait({
        video: template.video,
        productDatasetId: flow.productDatasetId,
      })
      .unwrap();

    updatedVideo.config.musicFile = getRandomMusicFile();
    updatedVideo.config.avatarFile = avatar.preview;
    ...
    await convex.mutation(api.private.flows.thought_leadership_916.update, {
      id: flowId,
      data: { templatedVideo: updatedVideo, voiceGenerating: true },
    });
  },
...
export const generateVoice = schemaTask({
  id: "flows/thought_leadership_916/generate-voice",
  schema: z.object({
    flowId: z.string().transform((val) => val as Id<"flows">),
  }),
  maxDuration: 300,
...
export const generateVideo = schemaTask({
  id: "flows/thought_leadership_916/generate-video",
  schema: z.object({
    flowId: z.string().transform((val) => val as Id<"flows">),
  }),
  maxDuration: 60 * 15,
