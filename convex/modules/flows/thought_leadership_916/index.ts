...
import { FlowType, FlowThoughtLeadership916 } from "@convex/schema/flows";
import {
...
export async function get(
  ctx: QueryCtx,
  args: { id: Id<"flows"> }
): Promise<FlowThoughtLeadership916 | null> {
  const { id } = args;

  const flow = await ctx.db.get(id);
  if (flow && flow.type === FlowType.ThoughtLeadership916) {
    return flow;
  }
...
export async function create(
  ...
  await retrier.run(
    ctx,
    internal.modules.flows.thought_leadership_916.index._generateTemplate,
    { id: flowId },
    { maxFailures: 2 }
  );
...
export async function generateVoice(
  ...
  await retrier.run(
    ctx,
    internal.modules.flows.thought_leadership_916.index._generateVoice,
    { id },
    { maxFailures: 2 }
  );
}
...
export async function generateVideo(
  ...
  await retrier.run(
    ctx,
    internal.modules.flows.thought_leadership_916.index._generateVideo,
    { id },
    { maxFailures: 2 }
  );
}
...
export async function fetchRequiredDataForTemplate(
  ...
  return {
    flow,
    avatar: avatar as ReadyAvatar & { aspect: "9:16" },
    workspace,
  };
}
...
export const _generateTemplate = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    await trigger.runTask<GenerateTemplateTask>(
      "flows/thought_leadership_916/generate-template",
      { flowId: id }
    );
  },
});
...
export const _generateVoice = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    await trigger.runTask<GenerateVoiceTask>(
      "flows/thought_leadership_916/generate-voice",
      { flowId: id }
    );
  },
});
...
export const _generateVideo = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    await trigger.runTask<GenerateVideoTask>(
      "flows/thought_leadership_916/generate-video",
      { flowId: id }
    );
  },
});
