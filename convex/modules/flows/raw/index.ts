...
import { FlowType, FlowRaw } from "@convex/schema/flows";
import {
...
export async function get(
  ctx: QueryCtx,
  args: { id: Id<"flows"> }
): Promise<FlowRaw | null> {
  const { id } = args;

  const flow = await ctx.db.get(id);
  if (flow && flow.type === FlowType.Raw) {
    return flow;
  }
...
export async function create(
  ...
  await retrier.run(
    ctx,
    internal.modules.flows.raw.index._generateTemplate,
    { id: flowId },
    { maxFailures: 2 }
  );
...
export async function generateVoice(
  ...
  await retrier.run(
    ctx,
    internal.modules.flows.raw.index._generateVoice,
    { id },
    { maxFailures: 2 }
  );
}
...
export async function generateVideo(
  ...
  await retrier.run(
    ctx,
    internal.modules.flows.raw.index._generateVideo,
    { id },
    { maxFailures: 2 }
  );
}
...
export async function fetchRequiredDataForTemplate(
  ...
  if (!avatar || avatar.status !== AvatarStatus.Ready || !avatar.preview) {
...
export const _generateTemplate = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    await trigger.runTask<GenerateTemplateTask>("flows/raw/generate-template", {
      flowId: id,
    });
  },
});
...
export const _generateVoice = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    await trigger.runTask<GenerateVoiceTask>("flows/raw/generate-voice", {
      flowId: id,
    });
  },
});
...
export const _generateVideo = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    await trigger.runTask<GenerateVideoTask>("flows/raw/generate-video", {
      flowId: id,
    });
  },
});
