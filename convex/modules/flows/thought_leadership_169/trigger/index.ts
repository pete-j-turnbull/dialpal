...
const generateVideoQueue = queue({
  name: "flows/thought_leadership_169/generate-video",
  concurrencyLimit: 10,
});

const generateBRoll = schemaTask({
  id: "flows/thought_leadership_169/generate-broll",
  schema: z.object({
    video: videoSchema,
    productDatasetId: z
      .string()
...
export const generateTemplate = schemaTask({
  id: "flows/thought_leadership_169/generate-template",
  schema: z.object({
  ...
  run: async (payload) => {
    ...
    const requiredData = await convex.query(
      api.private.flows.thought_leadership_169.fetchRequiredDataForTemplate,
      { id: flowId }
    );
    ...
    await convex.mutation(api.private.flows.thought_leadership_169.update, {
      id: flowId,
...
export const generateVoice = schemaTask({
  id: "flows/thought_leadership_169/generate-voice",
  schema: z.object({
    flowId: z.string().transform((val) => val as Id<"flows">),
  }),
  maxDuration: 300,
...
export const generateVideo = schemaTask({
  id: "flows/thought_leadership_169/generate-video",
  schema: z.object({
    flowId: z.string().transform((val) => val as Id<"flows">),
  }),
  maxDuration: 60 * 15,
